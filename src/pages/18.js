...
function Fundamentals8() {
  const allCards = data.cards;
  const [activeCard, setActiveCard] = useState(0);
  const [activeLink, setActiveLink] = useState(7); // Only need one activeLink state
  ...
  const handleCardSelect = (cardIndex) => {
    ...
    setActiveLink(0); // Reset to first link when card changes
